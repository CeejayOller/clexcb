...
export const isImportTransaction = (
  type: TransactionType
): type is ImportTransactionType => {
...
export const getNextReferenceNumber = async (
  transactionType: TransactionType
): Promise<ReferenceNumberResponse> => {
  try {
...
export const parseReferenceNumber = (referenceNumber: string) => {
  const match = referenceNumber.match(
    /^(CLEX)-(IMS|IMA|ACN|ACR|EXP)(\d{2})-(\d{4})$/
  );
